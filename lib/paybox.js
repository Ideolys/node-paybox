...
var paybox = {
  ...
  response          : function(transaction, datas, pubkeyPath, callback){
    var _error = null;
    paybox.checkIdentity(transaction, datas, pubkeyPath, function(identityOK){
      if(!identityOK){
  ...
  getKey            : function(pathOrKey, callback){
   ...
   * @return {Boolean}
   */
  checkIdentity     : function(transaction, datas, pubkeyPath, callback){
    paybox.getKey(pubkeyPath, function(pubkey){
      /*console.log(pubkey);*/
      ...
      if(_signField !== null){
        ...
        _check = paybox.checkSignature(_message, _sign, pubkey);
      }
      callback(_check);
    });
  },
  ...
  generateHMAC      : function(PBXFields, key){
    ...
    try{
      ...
      _hmac = paybox.generateSignature(_message, key);
    }
    catch(e){}
    return _hmac;
  },
  generateSignature : function(message, key){
    var _signature = null;
    try{
      var _key = (new Buffer(key, 'hex')).toString('binary');
      _signature = crypto.createHmac('sha512', _key).update(message).digest('hex').toUpperCase();
    }
    catch(e){}
    return _signature;
  },
